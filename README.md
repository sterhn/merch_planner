...
4. Repeat for each remaining file in [`supabase/migrations/`](supabase/migrations/) in numeric order (`002_bundle_items.sql`, `003_stock_sync.sql`, `004_product_photos.sql` — adds the item description + product photo columns and the `product-photos` storage bucket — then `005_bundle_stock.sql`, which makes selling a bundle consume its components' stock and removes the outdated delivery-method constraint).
