...
- **Surfaces are liquid glass.** `glass` (in `index.css`) is the panel utility: translucent `surface`, backdrop blur + saturate, a bright top rim, a diagonal sheen and `shadow-card`, all in one — don't add `bg-surface`/`shadow-*`/`ring-*` beside it (they fight it for the same properties). Tune opacity with `[--glass-fill:80%]`; flag a pane with `glass-alert` (red halo). The page behind is graph paper over a soft colour wash — the wash is what the glass picks up. Inputs and `Modal` sheets stay solid `bg-surface` for legibility (the modal's scrim is its backdrop root, so glass there would only blur grey). Filled brand buttons carry a `--glass-gloss` top edge. `backdrop-filter` makes an element a containing block for fixed descendants — never put `glass` on an ancestor of `Modal`.
